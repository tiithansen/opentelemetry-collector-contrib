...
func TestExemplarsForSumMetricsWithDynamicConfigAndAttributeSet(t *testing.T) {
	mcon := consumertest.NewNop()
	p := newConnectorImp(t, mcon, stringp("defaultNullValue"), explicitHistogramsConfig, enabledExemplarsConfigWithDynamicConfig, enabledEventsConfig, cumulative, zaptest.NewLogger(t), nil)
	traces := buildSampleTrace()
